...
bool isGrid (vec2 coord, float pitch, float proportion) {
  // return abs(mod(coord.x, pitch) - pitch) < 1. * C || abs(mod(coord.y, pitch) - pitch) < 1. * C;
  float thickness = pitch * proportion;
  return abs(mod(coord.x + thickness / 2., pitch)) < thickness
      || abs(mod(coord.y + thickness / 2., pitch)) < thickness;
}
